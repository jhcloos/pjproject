				...
				const pj_str_t *param_text)
{
	...
	pjsip_hdr *hdr = (pjsip_hdr *)pjsip_hdr_shallow_clone(tdata->pool, endpt_hdr);
    ...
    /* Create message body. */
    if (param_text) {
	body = (pjsip_msg_body*) pj_pool_calloc(tdata->pool, 1, sizeof(pjsip_msg_body));
				     ...
				     pjsip_tx_data **p_tdata)
{
	...
	from = (pjsip_fromto_hdr*) pjsip_hdr_clone(tdata->pool, param_from);
	pjsip_fromto_hdr_set_from(from);
	to = (pjsip_fromto_hdr*)pjsip_hdr_clone(tdata->pool, param_to);
						 ...
						 pjsip_tx_data **p_tdata)
{
	...
	if (via != (void*)&req_msg->hdr)
	    via = (pjsip_via_hdr*) pjsip_msg_find_hdr(req_msg, PJSIP_H_VIA, via);
	else
	    break;
    }

    /* Copy all Record-Route headers, in order. */
    rr = (pjsip_route_hdr*) pjsip_msg_find_hdr(req_msg, PJSIP_H_RECORD_ROUTE, NULL);
    while (rr) {
	pjsip_msg_add_hdr(msg, (pjsip_hdr*) pjsip_hdr_clone(tdata->pool, rr));
	rr = rr->next;
	if (rr != (void*)&req_msg->hdr)
	    rr = (pjsip_route_hdr*) pjsip_msg_find_hdr(req_msg, PJSIP_H_RECORD_ROUTE, rr);
	else
	    break;
    }
...
#   undef FIND_HDR
	...
	if (hdr != &cancel_tdata->msg->hdr)
	    hdr = (pjsip_hdr*) pjsip_msg_find_hdr(cancel_tdata->msg, PJSIP_H_ROUTE, hdr);
	else
	    break;
    }
					    ...
					    pjsip_host_info *dest_info )
{
	...
	(pjsip_route_hdr*) pjsip_msg_find_hdr(tdata->msg, PJSIP_H_ROUTE, NULL);
    if (first_route_hdr) {
					      ...
					      pj_bool_t *cont))
{
    ...
    /* Keep stateless data. */
    stateless_data = (pjsip_send_state *) 
		     pj_pool_zalloc(tdata->pool, sizeof(pjsip_send_state));
