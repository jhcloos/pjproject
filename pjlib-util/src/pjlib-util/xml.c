...
static pj_xml_node *alloc_node( pj_pool_t *pool )
{
    ...
    node = (pj_xml_node*) pj_pool_zalloc(pool, sizeof(pj_xml_node));
...
{
    return (pj_xml_attr*) pj_pool_zalloc(pool, sizeof(pj_xml_attr));
}
...
{
    pj_xml_node *head = (pj_xml_node *) &parent->node_head, 
	        *node = head->next;
